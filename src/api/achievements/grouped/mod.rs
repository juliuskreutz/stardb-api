...
async fn update(groups: &web::Data<Mutex<HashMap<Language, Groups>>>, pool: &PgPool) -> Result<()> {
...
async fn get_achievements_grouped(
    ...
    let groups = &groups.lock().await[&language_params.lang];
