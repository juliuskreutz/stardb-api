...
pub async fn get_by_uid(
    ...
    Ok(sqlx::query_file_as!(
        DbSignal,
        "sql/zzz/signals/exclusive_rescreening/get_by_uid.sql",
        uid,
        language
    )
    .fetch_all(pool)
...
pub async fn get_infos_by_uid(uid: i32, pool: &PgPool) -> anyhow::Result<Vec<DbSignalInfo>> {
    Ok(
        sqlx::query_file_as!(DbSignalInfo, "sql/zzz/signals/exclusive_rescreening/get_infos.sql", uid)
            .fetch_all(pool)
            .await?,
    )
}
...
pub async fn get_count_by_uid(uid: i32, pool: &PgPool) -> anyhow::Result<i64> {
    Ok(
        sqlx::query_file!("sql/zzz/signals/exclusive_rescreening/get_count_by_uid.sql", uid)
            .fetch_one(pool)
            .await?
            .count
            .unwrap(),
...
pub async fn delete_unofficial(uid: i32, pool: &PgPool) -> anyhow::Result<()> {
    sqlx::query_file!("sql/zzz/signals/exclusive_rescreening/delete_unofficial.sql", uid)
        .execute(pool)
        .await?;
