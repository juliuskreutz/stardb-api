//! ZZZ population-percentile persistence and pool-specific routing. This module targets the bangboo pool.
...
pub async fn set_bulk(stats: &[DbSignalsStatGlobal], pool: &PgPool) -> Result<()> {
    ...
    sqlx::query_file!(
        "sql/zzz/signals_stats_global/bangboo/set_bulk.sql",
        &uids,
        &count,
        &luck_a,
        &luck_s,
...
pub async fn delete_bulk(uids: &[i32], pool: &PgPool) -> Result<()> {
    ...
    sqlx::query_file!("sql/zzz/signals_stats_global/bangboo/delete_bulk.sql", uids)
        .execute(pool)
        .await?;
