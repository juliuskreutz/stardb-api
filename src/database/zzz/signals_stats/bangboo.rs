...
pub async fn update_percentiles_by_uid(
    ...
    sqlx::query_file!(
        "sql/zzz/signals_stats/bangboo/update_percentiles_by_uid.sql",
        uid,
        count_percentile,
        luck_a_percentile,
        luck_s_percentile
...
pub async fn set_data(set_data: &SetData, pool: &PgPool) -> Result<()> {
    sqlx::query_file!(
        "sql/zzz/signals_stats/bangboo/set_data.sql",
        set_data.uid,
        set_data.luck_a,
        set_data.luck_s,
    )
...
pub async fn count(pool: &PgPool) -> Result<i64> {
    Ok(sqlx::query_file!("sql/zzz/signals_stats/bangboo/count.sql")
        .fetch_one(pool)
        .await?
        .count
        .unwrap())
