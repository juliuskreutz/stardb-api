...
fn synthetic_stats_match_pre_refactor_every_pool() {
    ...
    for length in [0, 1, 2, 10, 90, 300] {
        let rows: Vec<_> = (0..length)
            .map(|i| baseline::Row {
                ...
                character: Some(if i % 2 == 0 { 10000042 } else { 9001 }),
    ...
    for length in [0, 1, 2, 10, 90, 300] {
        ...
        assert_eq!(
            ...
            baseline::character(&rows, &banners),
            "gi/character {length}"
        );
    }
    for length in [0, 1, 2, 10, 90, 300] {
        ...
        let scan = super::super::scan::scan_event(
            ...
            |r| {
                Some(banners.classify(
                    ...
                    PullItem::Weapon(r.weapon.unwrap()),
                    r.timestamp,
                ))
            },
            4,
    ...
    for length in [0, 1, 2, 10, 90, 300] {
        ...
        assert_eq!(
            ...
            baseline::chronicled(&rows, &banners),
            "gi/chronicled {length}"
        );
    }
}
