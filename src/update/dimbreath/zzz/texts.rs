...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["FAMMBPLAHCO"] {
        ...
        for achievement in &configs.achievement["FAMMBPLAHCO"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["FAMMBPLAHCO"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["FAMMBPLAHCO"] {
            let name = text_map
        ...
        for avatar in &configs.avatar["FAMMBPLAHCO"] {
            let name = text_map
                .get(&avatar.name)
                .cloned()
        ...
        for weapon in &configs.weapon["FAMMBPLAHCO"] {
            ...
            let name = &configs.item["FAMMBPLAHCO"]
                .iter()
        ...
        for buddy in &configs.buddy["FAMMBPLAHCO"] {
            let id = buddy.id;
