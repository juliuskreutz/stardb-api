...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["MIJCMCEDADM"] {
        ...
        for achievement in &configs.achievement["MIJCMCEDADM"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["MIJCMCEDADM"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["MIJCMCEDADM"] {
            let name = text_map
        ...
        for weapon in &configs.weapon["MIJCMCEDADM"] {
            let id = weapon.id;

            let name = &configs.item["MIJCMCEDADM"]
                .iter()
        ...
        for buddy in &configs.buddy["MIJCMCEDADM"] {
            let id = buddy.id;
