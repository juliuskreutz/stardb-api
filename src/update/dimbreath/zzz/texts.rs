...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["LDABBJOAKGJ"] {
        ...
        for achievement in &configs.achievement["LDABBJOAKGJ"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["LDABBJOAKGJ"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["LDABBJOAKGJ"] {
            let name = text_map
        ...
        for avatar in &configs.avatar["LDABBJOAKGJ"] {
            let name = text_map
                .get(&avatar.name)
                .cloned()
        ...
        for weapon in &configs.weapon["LDABBJOAKGJ"] {
            ...
            let name = &configs.item["LDABBJOAKGJ"]
                .iter()
        ...
        for buddy in &configs.buddy["LDABBJOAKGJ"] {
            let id = buddy.id;
