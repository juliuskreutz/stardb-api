...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["IBHONBJMECB"] {
        ...
        for achievement in &configs.achievement["IBHONBJMECB"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["IBHONBJMECB"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["IBHONBJMECB"] {
            let name = text_map
        ...
        for avatar in &configs.avatar["IBHONBJMECB"] {
            let name = text_map
                .get(&avatar.name)
                .cloned()
        ...
        for weapon in &configs.weapon["IBHONBJMECB"] {
            let id = weapon.id;

            let name = &configs.item["IBHONBJMECB"]
                .iter()
        ...
        for buddy in &configs.buddy["IBHONBJMECB"] {
            let id = buddy.id;
