...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["PDHBFPILAJD"] {
        ...
        for achievement in &configs.achievement["PDHBFPILAJD"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["PDHBFPILAJD"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["PDHBFPILAJD"] {
            let name = text_map
        ...
        for avatar in &configs.avatar["PDHBFPILAJD"] {
            let name = text_map
                .get(&avatar.name)
                .cloned()
        ...
        for weapon in &configs.weapon["PDHBFPILAJD"] {
            let id = weapon.id;

            let name = &configs.item["PDHBFPILAJD"]
                .iter()
        ...
        for buddy in &configs.buddy["PDHBFPILAJD"] {
            let id = buddy.id;
