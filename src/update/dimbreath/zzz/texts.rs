...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["IAEFFFFLKNB"] {
        ...
        for achievement in &configs.achievement["IAEFFFFLKNB"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["IAEFFFFLKNB"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["IAEFFFFLKNB"] {
            let name = text_map
        ...
        for avatar in &configs.avatar["IAEFFFFLKNB"] {
            let name = text_map
                .get(&avatar.name)
                .cloned()
        ...
        for weapon in &configs.weapon["IAEFFFFLKNB"] {
            let id = weapon.id;

            let name = &configs.item["IAEFFFFLKNB"]
                .iter()
        ...
        for buddy in &configs.buddy["IAEFFFFLKNB"] {
            let id = buddy.id;
