...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for (language_str, language) in [
        ...
        for achievement_second_class in &configs.achievement_second_class["EDFEHLNPDHN"] {
        ...
        for achievement in &configs.achievement["EDFEHLNPDHN"] {
        ...
        for arcade_achievement_group in &configs.arcade_achievement_group["EDFEHLNPDHN"] {
        ...
        for arcade_achievement in &configs.arcade_achievement["EDFEHLNPDHN"] {
            let name = text_map
        ...
        for avatar in &configs.avatar["EDFEHLNPDHN"] {
            let name = text_map
                .get(&avatar.name)
                .cloned()
        ...
        for weapon in &configs.weapon["EDFEHLNPDHN"] {
            let id = weapon.id;

            let name = &configs.item["EDFEHLNPDHN"]
                .iter()
        ...
        for buddy in &configs.buddy["EDFEHLNPDHN"] {
            let id = buddy.id;
