...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for achievement_data in &configs.achievement["FAMMBPLAHCO"] {
        let id = achievement_data.id;

        let series = achievement_data.series;

        let rewards = configs.once_reward["FAMMBPLAHCO"]
            .iter()
    ...
    for arcade_achievement_data in &configs.arcade_achievement["FAMMBPLAHCO"] {
