...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for achievement_data in &configs.achievement["MCOOHPLIKCF"] {
        let id = achievement_data.id;

        let series = achievement_data.series;

        let rewards = configs.once_reward["MCOOHPLIKCF"]
            .iter()
    ...
    for arcade_achievement_data in &configs.arcade_achievement["MCOOHPLIKCF"] {
