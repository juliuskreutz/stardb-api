...
pub async fn update(configs: &Configs, pool: &PgPool) -> anyhow::Result<()> {
    ...
    for achievement_data in &configs.achievement["MLOEFHJHCID"] {
        let id = achievement_data.id;

        let series = achievement_data.series;

        let rewards = configs.once_reward["MLOEFHJHCID"]
            .iter()
    ...
    for arcade_achievement_data in &configs.arcade_achievement["MLOEFHJHCID"] {
